  ...
  public void updateFightData() {
    ...
    try {
      ...
      for (List<String> fight : records) {
        if (fight.get(1).equals(winnerCharType) && fight.get(2).equals(winnerWeapon)
            && fight.get(4).equals(losserCharType) && fight.get(5).equals(looserWeapon)) {
          fight.set(0, String.valueOf(1 + Integer.parseInt(fight.get(0))));
          found = true;
        } else if (fight.get(4).equals(winnerCharType) && fight.get(5).equals(winnerWeapon)
            && fight.get(1).equals(losserCharType) && fight.get(2).equals(looserWeapon)) {
